...
if st.button("Get Started!") or st.session_state['function_name'] is not None:
    ...
    st.session_state['function_name'] = st.text_input("Name of the function to generate:", st.session_state['function_name'])
...
if st.session_state['function_name'] is not None:
    if st.button("Build a scraper!"):
        with st.spinner('Scannning the website to learn how to scrape it (this will take a minute)'):
            st.session_state['scraping_instructions'] = learn_to_scrape(url, task)
            
        with st.spinner('Writing a scraper with those instructions'):
            st.session_state['scraper'] = write_scraper(url, task, st.session_state['function_name'], st.session_state['scraping_instructions'])
...
if st.session_state['scraper'] is not None:
    st.write("#### Here's a first draft of your scraper -- feel free to edit it, then you can automatically debug it below")
    st.session_state['scraper'] = st_ace(value=st.session_state['scraper'], language='python', theme='ambiance')


if st.session_state['scraper'] is not None:
    st.header('Automatic Debugger')
    st.write("When you're ready, have GPT-4 iteratively debug the above code to make sure it works")
    num_rounds = st.slider('Maximum Debugging Rounds', 1, 50, 5, 1)
    debug_model = st.selectbox('What model to use for debugging', ['gpt-4', 'gpt-3.5-turbo'])
    ...
    creation_log = [
        ...
        {"role": "user", "content": write_scraper_prompt(url, task, st.session_state['function_name'], st.session_state['scraping_instructions'])},
    ...
    if st.button('Start Debugging'):
        ...
        for i in range(num_rounds):
            ...
            with code_col:
                st_ace(key=f'debug_round_{i}', value=working_code, language='python', theme='ambiance', readonly=True)
            with output_col:
            ...
            if return_code == 0: # it worked!
                ...
                working_code, creation_log, response = try_debug(working_code, creation_log, st.session_state["function_name"], output, err, model=debug_model)
                with output_col:
...
if st.session_state['final_scraper'] is not None:
    ...
    st_ace(value=st.session_state['final_scraper'], language='python', theme='ambiance', key='final_scraper', readonly=True)