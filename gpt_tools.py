...
def write_scraper(url, task, function_name, instructions):
    completion = chat_gpt_with_backoff(
      ...
      messages=[
            {"role": "system", "content": system_prompt()},
            {"role": "user", "content": write_scraper_prompt(url, task, instructions, function_name)}
      ]
    )
...
def try_debug(code, log, function_name, stdout, stderr, model='gpt-4'):
    prompt = debug_error_prompt(function_name=function_name, stdout=stdout, stderr=stderr)
